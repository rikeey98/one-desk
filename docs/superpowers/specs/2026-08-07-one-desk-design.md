...
> **2026-09-07 구현하며 정한 것** (`2026-09-07-asset-scan-design.md`).
> - §156의 `asset` 스케치에 **`updated_at`이 빠져 있다.** `authored` 본문 편집에
>   이슈·메모와 같은 낙관적 잠금이 필요하고, 그것은 이 컬럼 없이 성립하지 않는다(§2-1).
> - 스캔 시점에 **"앱을 열 때"를 더한다**(§3-2). 없으면 "어제 만든 skill이 안 보인다"가
>   가장 흔한 불편이 된다.
> - 이 문서가 정하지 않은 **동일성 키는 `(workspace_id, repo_id, file_path)`** 이고
>   UNIQUE를 건다(§3-3). 없으면 스캔마다 같은 파일이 새 행으로 쌓인다.
