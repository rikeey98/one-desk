...
| 완료 · 미확인 | *(변경 보기 — 5단계)* · 이어서 실행 · 확인함 |
| 실패 | 로그 보기 · 다시 실행 · 이슈로 만들기 · 보관 |
| 중단됨 | 로그 보기 · 다시 실행 · 보관 |
| 대기 중 취소됨 | 다시 실행 · 보관 (†) |

**(†) 갱신 — 대화 단위로 바뀌며 뒤집혔다.** 이 표와 아래 "로그 보기를 넣지 않는다"는 판정 단위가 run이던 시절 것이다. `2026-08-18-conversation-design.md`(§5)로 항목 단위가 대화(root run)로 바뀌면서, "로그 보기"와 "이어서 실행"이 **"대화 열기"** 하나로 합쳐졌고 그 버튼은 카테고리와 무관하게 전부 뜬다(리뷰 I-3) — 취소된 마지막 턴 자체는 로그가 없어도, **그 대화의 앞선 턴들에는 대화록이 있을 수 있기 때문이다.** 그래서 지금 "대기 중 취소됨"의 실제 행동은 **대화 열기 · 다시 실행 · 보관**이다. 현재 값은 `renderer/components/InboxPanel.tsx`의 `shows()`가 근거다.

**관련 이슈 닫기**는 이 표에 없다 — 카테고리에 속한 행동이 아니라 **첨부된 이슈 하나하나에 속한 행동**이기 때문이다. run에 이슈가 붙어 있으면 카테고리와 무관하게 이슈마다 하나씩 뜬다. 실패하거나 중단된 run에 붙은 이슈도 사용자가 "이건 이미 해결됐다"고 판단할 수 있어야 하므로, 완료·미확인으로 좁히지 않는다.
...
**로그 보기를 넣지 않는다.** 시작도 못 한 run이라 로그 파일이 없다. *(갱신: 항목 단위가 run에서 대화로 바뀌며 이 판단이 뒤집혔다 — 위 후속 행동표의 (†) 참고.)*
