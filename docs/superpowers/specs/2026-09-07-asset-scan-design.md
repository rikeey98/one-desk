...
> **2026-09-19 보충.** "이번 스캔에서 보이지 않은 것"은 화면에서 `lastSeenAt`이 그 workspace의
> 최댓값보다 작은 것으로 판정한다. 그래서 **한 번의 `scanWorkspace`는 시각 하나를 잡아 모든
> 배치(repo마다·글로벌 루트마다)에 같은 `last_seen_at`을 찍는다.** 배치마다 따로 찍으면 먼저
> 훑은 repo의 asset이 나중에 훑은 글로벌보다 몇 ms 오래돼 방금 본 파일이 "없음"이 됐다.
> `scanRepo`는 그 repo만 훑는 부분 스캔이므로 이 판정과 어긋날 수 있다 — §3-2의 결정이라
> 여기서 바꾸지 않고 남겨 둔다.
...
> **2026-09-21 보강.** 위 "읽기 전용으로 보여준다"는 구현이 한동안 자리표시자("본문은 실행
> 시점에 이 파일에서 읽습니다")에 멈춰 있었다. 이제 상세를 열면 `assets.readBody(id)`가
> 파일의 지금 내용을 읽어 보여준다 — 저장하지 않는 §2-2는 그대로다. 같은 작업에서 repo
> 루트의 `CLAUDE.md`·`AGENTS.md`가 세 번째 종류 `instructions`로 목록에 오르되 **맥락에는
> 담을 수 없다**(CLI가 알아서 읽는다). 근거는 `docs/sdlc/repo-instructions/`.
