...
| 폐기 | `launch()`의 `mcp.prepare` catch | 설정 파일 쓰기가 실패해도 토큰은 이미 등록돼 있다 |
| 폐기 | `finish()`의 `finally` | 정상 종료·실패·시간 초과·spawn 거부 |
| 폐기 | `beginRun()`의 catch (유령 run) | 시작 기록이 실패한 run |
| 폐기 | `cancel()`의 대기 분기 | **슬롯을 쥔 적이 없는데 토큰은 쥐고 있다** |
| 폐기 | `shutdown()` | 맵 전체를 비운다 |

**`prepare()`로 토큰을 받은 run을 포기하는 모든 자리가 토큰을 폐기하는 자리다.**

이 문장은 원래 "슬롯을 돌려주는 모든 자리"였다. **그 규칙은 틀렸고, 실제로 구멍을 하나 남겼다** — 토큰은 `enqueue` **앞에서** 발급되므로 대기 중인 run도 살아 있는 토큰을 쥔다. 그런데 대기 중 취소는 슬롯을 쥔 적이 없어서 `queue.release`를 부르지 않고, 따라서 슬롯을 기준으로 삼은 규칙이 그 경로에 닿지 않았다. 4단계 리뷰가 잡았다. 새 종료 경로를 만들 때 기준은 슬롯이 아니라 **토큰을 받았는가**다.

발급을 `enqueue` 앞에 두는 이유는 대기 중인 run도 설정 파일이 준비돼 있어야 하고, 슬롯을 얻는 순간 곧바로 프로세스를 띄우기 때문이다. 대기 중 토큰이 살아 있는 것 자체는 문제가 아니다 — 그 토큰을 쥔 프로세스가 아직 없다. 문제는 **그 run이 프로세스를 띄우지 못한 채 끝나는 경로**이고, 위 표의 둘째 줄과 다섯째 줄이 그것이다.
