...
- 커밋 메시지는 영어, 명령형.
- **의도된 중복 — 합치지 말 것.** `issue.ts`↔`memo.ts`, 그리고 이번에 생기는 `loadIssue`↔`loadMemo`는 거의 같은 코드다. **실수가 아니라 사용자가 명시적으로 승인한 설계 결정이다** — 이슈에는 앞으로 상태 전이와 run 연결이 붙지만 메모에는 붙지 않는다. 공통 헬퍼로 추출하자는 제안은 결함 지적이 아니다. **대신 두 쌍을 항상 대칭으로 유지한다.** 한쪽을 고치면 반드시 다른 쪽도 고친다. 어긋나면 그건 진짜 결함이다.
- **`ResolvedRunSpec.mcp`의 `token`·`url`은 어댑터가 쓰지 않는데 일부러 담는다.** 담지 않으면 "토큰이 인자에 안 나온다" 테스트가 구조적으로 실패할 수 없는 무력한 테스트가 된다 (Task 5 참고). 미사용 필드로 지적하지 않는다.
...
- [ ] **Step 3: `core/mcp/configFile.test.ts`를 쓴다 (실패 확인)**
...
Expected: FAIL — `./configFile` 모듈이 없다.

- [ ] **Step 4: `core/mcp/configFile.ts`를 구현한다**
...
**모드 테스트가 진짜인지 확인한다** — `writeFileSync`의 `{ mode: 0o600 }`을 지우고 다시 돌려 실패하는 것을 본 뒤 되돌린다. umask에 따라 0644가 나온다.
