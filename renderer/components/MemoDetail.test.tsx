...
function makeMemo(over: Partial<Memo> = {}): Memo {
  return {
    id: 'm1', workspaceId: 'w1', title: '토큰 만료', body: '원본',
...
describe('MemoDetail', () => {
  ...
  it('본문을 고치면 기대 updatedAt과 함께 저장한다', async () => {
    ...
    expect(client.memos.updateIfUnchanged).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'm1', body: '원본!', expectedUpdatedAt: 100 })
    )
  })
  ...
  it('성공한 저장이 기대값을 갱신해 두 번째 저장이 충돌하지 않는다', async () => {
    ...
    const calls = vi.mocked(client.memos.updateIfUnchanged).mock.calls
  ...
  it('충돌하면 배너를 띄우고 자동 저장을 멈춘다', async () => {
    ...
    await userEvent.type(screen.getByLabelText('본문'), '?')
    await act(async () => { await vi.advanceTimersByTimeAsync(600) })
    expect(client.memos.updateIfUnchanged).toHaveBeenCalledTimes(1)
  })
  ...
  it('다시 불러오기가 대기 중인 저장을 취소해 되돌린 내용을 몰래 덮어쓰지 않는다', async () => {
    ...
    vi.mocked(client.memos.updateIfUnchanged).mockClear()
    await act(async () => { await vi.advanceTimersByTimeAsync(600) })
  ...
  it('다시 불러오기 뒤의 저장은 새 기대값을 쓴다', async () => {
    ...
    const updateIfUnchanged = vi.fn()
      ...
      .mockResolvedValue({
        ok: true as const, memo: makeMemo({ body: 'agent가 쓴 것 더', updatedAt: 400 })
      })
  ...
  it('덮어쓰기가 잠금 없는 update를 부른다', async () => {
    ...
    expect(client.memos.update).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'm1', body: '원본!' })
    )
  })
  ...
  it('삭제는 두 번 눌러야 하고, 지워지면 알린다', async () => {
    ...
    expect(client.memos.remove).not.toHaveBeenCalled()
    await userEvent.click(screen.getByRole('button', { name: '정말 삭제?' }))
    expect(client.memos.remove).toHaveBeenCalledWith('m1')
