...
describe('MemoRepository', () => {
  ...
  describe('여러 repo에 걸친 항목', () => {
    it('두 repo에 태그하면 어느 쪽으로 걸러도 나온다', () => {
      ...
      memos.create({ workspaceId, title: '양쪽', repoIds: [apiRepoId, webRepoId] })
      memos.create({ workspaceId, title: 'api만', repoIds: [apiRepoId] })

      const fromApi = memos.list({ workspaceId, repoId: apiRepoId }).map((r) => r.title)
      const fromWeb = memos.list({ workspaceId, repoId: webRepoId }).map((r) => r.title)
