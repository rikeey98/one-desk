...
describe('McpHost', () => {
  ...
  it('동시에 두 run을 준비해도 포트를 하나만 연다', async () => {
    ...
    const [a, b] = await Promise.all([
      host.prepare({ runId: 'r1', workspaceId, permission: 'edit', agentKind: 'claude-code' }),
      host.prepare({ runId: 'r2', workspaceId, permission: 'read_only', agentKind: 'claude-code' })
    ])
  ...
  it('설정 파일의 mcpServers 키와 --allowedTools의 mcp__ 접두사가 같은 값에서 나온다', async () => {
    ...
    const p = await host.prepare({ runId: 'r-name', workspaceId, permission: 'edit', agentKind: 'claude-code' })
